...
//The program file already has an introductory paragraph
...
int is_leaf_node(node_t* ptr) {
  ...
  if(ptr->left == NULL && ptr->right == NULL){  //A node with NULL values on both left and right pointers is a leaf node.
    return 1;
  }
  return 0;
}
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  if(is_leaf_node(ptr) == 0){ //A node is either an internal node or a leaf node
    return 1;
  }
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  return is_in_subtree(a->left, b) || is_in_subtree(a->right, b); //use recursion
}
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if(ptr->cutline == V){  //if it was originally V, change to H
    ptr->cutline = H;
  }
  else{
    ptr->cutline = V;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  if(ptr->cutline != UNDEFINED_CUTLINE){  //if it is a cutline
    expression[*nth].cutline = ptr->cutline;
  }
  else{
    expression[*nth].module = ptr->module;
  }
  *nth = *nth + 1;      //increment nth(array index)
}
...
// Return 1 if any overlaps among momdules, and 0 otherwise.
int is_overlapped() {
  int i, j;
